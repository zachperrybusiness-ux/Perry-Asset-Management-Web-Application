...
async function fetchChart(t, range, interval) {
  const r = await fetch(WORKER_URL + "/chart?symbol=" + encodeURIComponent(t) + "&range=" + range + "&interval=" + (interval || "1d"));
  const d = await r.json();
  if (d.error) throw new Error(d.error);
  return d;
}
...
// ═══ NAVIGATION ═══
const parentMap = { home: 'home', about: 'home', portfolio: 'portfolio', holdings: 'portfolio', research: 'research', macro: 'macro', quant: 'quant' };
...
const FEATURES = [
  ...
  { title: "Quantitative Models", desc: "Stock decision trees, MLR regression, peer comparison, Monte Carlo simulation, and regime backtesting.", page: "quant", tag: "Active" },
  { title: "Risk & Stress Testing", desc: "VaR, Monte-Carlo, tracking error, scenario analysis.", page: null, tag: "Coming Soon" }
];
...
function sortableHeader(cols, sortState, renderFn, tableId) {
  return cols.map(c => {
    ...
    return '<th onclick="sortTable_' + tableId + '(\'' + c.key + '\')">' +
      c.label + '<span class="sort-arrow ' + active + '">' + arrow + '</span></th>';
  }).join('');
}

function getSortedHoldings(holdings, sortState) {
  if (!sortState.col) return holdings;
  const key = sortState.col;
  return [...holdings].sort((a, b) => {
    let va = a[key], vb = b[key];
    if (key === 'marketValue') {
      va = (a.currentPrice || 0) * a.quantity;
      vb = (b.currentPrice || 0) * b.quantity;
    }
    if (key === 'glPct') {
      const ca = a.costBasis * a.quantity, cb = b.costBasis * b.quantity;
      va = ca > 0 ? (((a.currentPrice || 0) * a.quantity - ca) / ca) * 100 : 0;
      vb = cb > 0 ? (((b.currentPrice || 0) * b.quantity - cb) / cb) * 100 : 0;
    }
    if (key === 'weight') {
      const tv = holdings.reduce((s, h) => s + (h.currentPrice || 0) * h.quantity, 0);
      va = tv > 0 ? ((a.currentPrice || 0) * a.quantity) / tv : 0;
      vb = tv > 0 ? ((b.currentPrice || 0) * b.quantity) / tv : 0;
    }
...
window.sortTable_portfolio = function(key) {
  if (_portfolioSort.col === key) _portfolioSort.asc = !_portfolioSort.asc;
  else { _portfolioSort.col = key; _portfolioSort.asc = true; }
  const h = window._holdings || [];
  const af = document.getElementById('portfolioAccountFilter')?.value || 'all';
  const fh = af === 'all' ? h : h.filter(x => (x.accountType || 'Individual') === af);
  const tv = fh.reduce((s, x) => s + (x.currentPrice || 0) * x.quantity, 0);
  renderPortfolioTable(fh, tv);
};
...
function renderHoldingsTable(holdings) {
  ...
  let html = '<table><thead><tr>' + sortableHeader(cols, _holdingsSort, null, 'manage') + '<th></th></tr></thead><tbody>';
  sorted.forEach(h => {
    ...
    html += '<tr>' +
      ...
      '<td style="font-weight:600;">' + fmt(isCash ? h.costBasis * h.quantity : mv) + '</td>' +
...
async function renderPortfolioOverview() {
  ...
  renderDonut('donut-industry', 'Portfolio Allocation by Industry', group('industry').slice(0, 10));
  renderDonut('donut-mktcap', 'Portfolio by Market Cap / Style', group('mktCapCategory'));
  renderDonut('donut-weight', 'Portfolio Allocation by Holding',
    holdings.map(h => ({
      label: h.ticker,
  ...
  document.getElementById('btnVixRange').classList.toggle('active', _showVixRange);
...
// ═══ PORTFOLIO TABLE (with Account, Sort, Total) ═══
function renderPortfolioTable(holdings, tv) {
  ...
  const cols = [
    {key:'ticker',label:'Ticker'}, {key:'companyName',label:'Company'}, {key:'quantity',label:'Shares'},
    {key:'currentPrice',label:'Price'}, {key:'marketValue',label:'Mkt Value'}, {key:'weight',label:'Weight'},
    {key:'glPct',label:'G/L'}, {key:'accountType',label:'Account'}, {key:'sector',label:'Sector'},
    {key:'industry',label:'Industry'}, {key:'assetClass',label:'Asset'}
  ];
  let html = '<table><thead><tr>' + sortableHeader(cols, _portfolioSort, null, 'portfolio') + '</tr></thead><tbody>';
  sorted.forEach(h => {
    const mv = (h.currentPrice || 0) * h.quantity;
    const cost = h.costBasis * h.quantity;
    const gl = mv - cost;
    ...
    html += '<tr>' +
      ...
      '<td>' + pp.toFixed(1) + '%</td>' +
      '<td style="color:' + gc + ';font-weight:600;">' + fmtPct(glp) + '</td>' +
      '<td>' + (h.accountType || 'Individual') + '</td>' +
      '<td>' + (h.sector || '—') + '</td>' +
      '<td>' + (h.industry || '—') + '</td>' +
      '<td>' + ad + '</td>' +
      '<td>' + (h.mktCapCategory || '—') + '</td></tr>';
  });
  // Total row
  html += '<tr style="font-weight:700;background:var(--panel);">' +
    '<td style="color:' + C.navy + ';">TOTAL</td><td>' + sorted.length + ' positions</td>' +
...
// ═══ STRESSED SCENARIOS & VIX OVERLAY ═══
...
window.toggleStress = function() {
  ...
  if (_showStress) _showVixRange = false;
  document.getElementById('btnStress').classList.toggle('active', _showStress);
  document.getElementById('btnVixRange').classList.toggle('active', _showVixRange);
  if (portfolioLineChart) portfolioLineChart.update();
};
window.toggleVixRange = function() {
  _showVixRange = !_showVixRange;
  if (_showVixRange) _showStress = false;
  document.getElementById('btnVixRange').classList.toggle('active', _showVixRange);
  document.getElementById('btnStress').classList.toggle('active', _showStress);
  if (portfolioLineChart) portfolioLineChart.update();
};
...
function drawRegion(ctx, xScale, si, ei, top, bottom, ev) {
  ...
  if (w > 30) {
    ...
    ctx.fillText(w > 80 ? ev.name : ev.name.split(' ')[0], x1 + w / 2, top + 4, w - 4);
  }
}
function drawVixRegion(ctx, xScale, si, ei, top, bottom, zone, name) {
  ...
  const x2 = xScale.getPixelForValue(ei);
  const w = x2 - x1;
  ctx.fillStyle = zone === 'high' ? 'rgba(139,105,20,0.15)' : 'rgba(200,208,216,0.2)';
  ctx.fillRect(x1, top, w, bottom - top);
  if (w > 30) {
    ctx.fillStyle = zone === 'high' ? 'rgba(139,105,20,0.7)' : 'rgba(90,106,122,0.6)';
    ctx.font = (w > 80 ? '600 10px' : '600 8px') + ' Arial';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    ctx.fillText(w > 80 ? name : (zone === 'high' ? 'VIX>30' : 'VIX>20'), x1 + w / 2, top + 4, w - 4);
  }
}
...
const overlayPlugin = {
  id: 'stressVixOverlay',
  afterDatasetsDraw(chart) {
    if (!_showStress && !_showVixRange) return;
    ...
    try {
      ...
      if (_showVixRange) {
        let cz = null, zs = null;
        for (let i = 0; i < dates.length; i++) {
          const vix = _vixDateMap[dates[i]];
          let z = null;
          if (vix != null) { if (vix >= 30) z = 'high'; else if (vix >= 20) z = 'elevated'; }
          if (z !== cz) {
            if (cz && zs !== null) {
              const ev = getVixEventForDate(dates[zs]) || { name: 'VIX ' + (cz === 'high' ? '>30' : '20-30') };
              drawVixRegion(ctx, x, zs, i - 1, top, bottom, cz, ev.name);
            }
            cz = z; zs = z ? i : null;
          }
        }
        if (cz && zs !== null) {
          const ev = getVixEventForDate(dates[zs]) || { name: 'VIX ' + (cz === 'high' ? '>30' : '20-30') };
          drawVixRegion(ctx, x, zs, dates.length - 1, top, bottom, cz, ev.name);
        }
      }
...
window.renderPortfolioChart = async function() {
  ...
  for (const h of holdings) {
    try {
      ...
      hs[h.ticker] = { map: m, quantity: h.quantity };
    } catch(e) {}
  }

  const ad = new Set();
  Object.values(hs).forEach(s => Object.keys(s.map).forEach(d => ad.add(d)));
  const dates = Array.from(ad).sort();
  if (!dates.length) { loadEl.style.display = 'none'; return; }

  // Portfolio value per date
  const pv = dates.map(d => {
    let t = 0;
    Object.values(hs).forEach(s => { if (s.map[d] != null) t += s.map[d] * s.quantity; });
    return t;
  });
  const valid = [];
  dates.forEach((d, i) => { if (pv[i] > 0) valid.push({ date: d, value: pv[i] }); });
  if (!valid.length) { loadEl.style.display = 'none'; return; }

  const cd = valid.map(d => d.date);
  const cv = valid.map(d => d.value);
  _chartDatesGlobal = cd;
  _chartPortfolioSeries = cv;
  ...
  const pfS = cv[0], pfE = cv[cv.length - 1];
  const ds = [{
    label: 'Portfolio', data: cv, borderColor: C.navy, backgroundColor: 'rgba(0,60,113,.06)',
    borderWidth: 2.5, pointRadius: 1.5, pointHoverRadius: 6,
  ...
  const hb = ds.length > 1;
  const pfRet = pfS > 0 ? ((pfE - pfS) / pfS * 100) : 0;
  let bmRet = null, bmSym = ci[0] || '';
  if (bmSym && _chartBenchmarkSeries[bmSym]) {
    const bd = Object.keys(_chartBenchmarkSeries[bmSym]).sort();
    if (bd.length >= 2) {
      const bf = _chartBenchmarkSeries[bmSym][bd[0]];
      const bl = _chartBenchmarkSeries[bmSym][bd[bd.length - 1]];
      bmRet = bf > 0 ? ((bl - bf) / bf * 100) : 0;
    }
  }
  const alpha = bmRet != null ? (pfRet - bmRet) : null;
  const gPf = pfS > 0 ? (10000 * (pfE / pfS)) : 10000;
  const gBm = bmRet != null ? (10000 * (1 + bmRet / 100)) : null;

  // Risk
  const dr = [];
  for (let j = 1; j < cv.length; j++) { if (cv[j - 1] > 0) dr.push((cv[j] - cv[j - 1]) / cv[j - 1]); }
  ...
  let sh = '';
  if (alpha != null) {
    sh += '<div class="chart-stat-box"><div class="chart-stat-label">Alpha vs ' + bmSym + '</div>' +
      '<div class="chart-stat-value" style="color:' + pctColor(alpha) + ';">' + fmtPct(alpha) + '</div>' +
      '<div class="chart-stat-sub">Portfolio ' + fmtPct(pfRet) + ' vs ' + bmSym + ' ' + fmtPct(bmRet) + '</div></div>';
  }
  sh += '<div class="chart-stat-box"><div class="chart-stat-label">Growth of $10,000</div>' +
    '<div class="chart-stat-value">Portfolio: ' + fmtInt(gPf) + '</div>' +
    '<div class="chart-stat-sub">' + (gBm != null ? bmSym + ': ' + fmtInt(gBm) : 'Add benchmark') + '</div></div>';
  sh += '<div class="chart-stat-box"><div class="chart-stat-label">Risk (Annualized)</div>' +
    '<div class="chart-stat-value">&#963; ' + sd.toFixed(1) + '%</div></div>';
    ...
    options: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function(c) {
              ...
              return c.dataset.yAxisID === 'y' ? ' Portfolio: ' + fmt(v) : ' ' + c.dataset.label + ': ' + fmt(v);
            },
            afterBody: function(items) {
              ...
              if (_showStress) {
                ...
                if (ev) {
                  ...
                  const ep = Math.max(...cv.slice(Math.max(0, es - 5), ee > 0 ? ee : cv.length));
                  const et = Math.min(...cv.slice(es, ee > 0 ? ee : cv.length));
              ...
              ci.forEach(sym => {
                ...
                if (!pm) return;
                const bd = Object.keys(pm).sort();
                if (bd.length < 2) return;
                const bf = pm[bd[0]], ba = pm[cd[idx]];
                if (bf > 0 && ba != null) lines.push('  ' + sym + ': ' + fmtPct(((ba - bf) / bf) * 100));
              });
              return lines;
            }
          }
      ...
      scales: {
        ...
        y: {
          ...
          ticks: { ...chartTicks, callback: function(v) { return '$' + (v >= 1e6 ? (v/1e6).toFixed(1)+'M' : v >= 1e3 ? (v/1e3).toFixed(0)+'K' : v.toFixed(0)); } },
          border: { display: false },
          title: { display: true, text: 'Portfolio Value ($)', font: { family: 'Arial', size: 12, weight: '600' }, color: C.textSec }
        },
        y1: {
          type: 'linear', position: 'right', display: hb,
          grid: { drawOnChartArea: false },
          ticks: { ...chartTicks, callback: function(v) { return '$' + v.toFixed(2); } },
          border: { display: false },
          title: { display: hb, text: 'Benchmark Price ($)', font: { family: 'Arial', size: 12, weight: '600' }, color: C.textSec }
        },
        yVol: {
...
function renderDonut(id, title, data) {
  ...
  if (!el) return;
  const cid = id + '-c';
  el.innerHTML = '<div class="card"><div class="card-title">' + title + '</div><div class="card-body">' +
    '<div class="donut-wrap"><div class="donut-canvas-wrap"><canvas id="' + cid + '"></canvas></div>' +
    '<div class="donut-legend">' +
    data.map((d, i) =>
      '<div class="legend-row"><span class="legend-dot" style="background:' + PALETTE[i % PALETTE.length] + '"></span>' +
      '<span class="legend-label">' + d.label + '</span><span class="legend-val">' + d.value + '%</span></div>'
    ).join('') +
    '</div></div></div><div class="card-sources"><strong>Sources:</strong><br>&#8226; Yahoo Finance &#8226; Firestore</div></div>';
  ...
  donutCharts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { ...chartTooltip, callbacks: { label: c => ' ' + c.label + ': ' + c.parsed + '%' } }
      }
    }
  });
}
...
async function processFile(file) {
  ...
  try {
    if (ext === 'csv' || ext === 'txt') {
      ...
      _bulkRows = parseGrid(text.split(/\r?\n/).map(l => l.split(/[,\t]/)));
    } else if (ext === 'xlsx' || ext === 'xls') {
      const buf = await file.arrayBuffer();
      const wb = XLSX.read(buf);
      const ws = wb.Sheets[wb.SheetNames[0]];
      _bulkRows = parseGrid(XLSX.utils.sheet_to_json(ws, { header: 1 }));
    } else {
    ...
    showStatus('scanStatus', '&#10003; Found ' + _bulkRows.length + ' holdings from <strong>' + file.name + '</strong>. Review below.', 'success');
...
window.submitBulkHoldings = async function() {
  ...
  for (const r of valid) {
    try {
      ...
      await window._addHoldingDirect(r.ticker, q.name || r.ticker, r.date, r.shares, r.cost, q.price || 0, q.previousClose || 0, meta, acctType);
      n++;
...
window.updateResearchChart = async function() {
  ...
  try {
    ...
    for (var bk of [b1, b2]) {
      if (bk && bk !== researchTicker) {
        try {
          ...
          var bmap = {};
          bpts.forEach(function(p) { bmap[p.date.slice(0, 10)] = p.close; });
          benchData[bk] = bmap;
        } catch(e) {}
      }
    }
    ...
    var labels = mainPts.map(function(p) { return p.date.slice(0, 10); });
...
function renderSECAnalysis(ticker, d) {
  ...
  function finTable(title, items, source) {
    if (!items || !items.length) return '';
    // Find all unique years across all items
    var allYears = {};
    items.forEach(function(item) { if (item.data) item.data.forEach(function(d) { allYears[d.year] = true; }); });
    var years = Object.keys(allYears).sort().slice(-5);
    if (!years.length) return '';

    var t = '<div class="card"><div class="card-title">' + title + '</div><div class="card-body"><div style="overflow-x:auto;">';
    t += '<table style="font-size:12px;white-space:nowrap;"><thead><tr><th style="min-width:160px;">Line Item</th>';
    years.forEach(function(y) { t += '<th style="text-align:right;">' + y + '</th>'; });
    t += '</tr></thead><tbody>';
    items.forEach(function(item) {
      if (!item.label) return;
      t += '<tr><td style="font-weight:600;">' + item.label + '</td>';
      years.forEach(function(y) {
        var found = null;
        if (item.data) item.data.forEach(function(d) { if (d.year === y) found = d.value; });
        if (found == null) { t += '<td style="text-align:right;color:'+C.textSec+';">—</td>'; }
        else if (item.isEPS) { t += '<td style="text-align:right;">$' + Number(found).toFixed(2) + '</td>'; }
        else if (Math.abs(found) >= 1e9) { t += '<td style="text-align:right;">' + (found/1e9).toFixed(2) + 'B</td>'; }
        else if (Math.abs(found) >= 1e6) { t += '<td style="text-align:right;">' + (found/1e6).toFixed(1) + 'M</td>'; }
        else { t += '<td style="text-align:right;">' + Number(found).toLocaleString() + '</td>'; }
      });
      t += '</tr>';
    });
    t += '</tbody></table></div></div><div class="card-sources"><strong>Sources:</strong><br>&#8226; ' + (source || 'SEC EDGAR 10-K Filings (XBRL)') + '</div></div>';
    return t;
  }

  h += finTable('Income Statement (Annual)', [
    { label: 'Revenue', data: inc.revenue },
    { label: 'Cost of Revenue', data: inc.costOfRevenue },
    { label: 'Gross Profit', data: inc.grossProfit },
    { label: 'R&D Expense', data: inc.researchAndDev },
    { label: 'SG&A Expense', data: inc.sgAndA },
    { label: 'Operating Income', data: inc.operatingIncome },
    { label: 'Net Income', data: inc.netIncome },
    { label: 'EPS (Diluted)', data: inc.eps, isEPS: true }
  ]);

  h += finTable('Balance Sheet (Annual)', [
    { label: 'Total Assets', data: bs.totalAssets },
    { label: 'Current Assets', data: bs.currentAssets },
    { label: 'Cash & Equivalents', data: bs.cash },
    { label: 'Total Liabilities', data: bs.totalLiabilities },
    { label: 'Current Liabilities', data: bs.currentLiabilities },
    { label: 'Long-Term Debt', data: bs.longTermDebt },
    { label: 'Stockholders Equity', data: bs.stockholdersEquity },
    { label: 'Retained Earnings', data: bs.retainedEarnings },
    { label: 'Shares Outstanding', data: bs.sharesOutstanding }
  ]);

  h += finTable('Cash Flow Statement (Annual)', [
    { label: 'Operating Cash Flow', data: cf.operatingCashFlow },
    { label: 'Capital Expenditures', data: cf.capitalExpenditures },
    { label: 'Stock Repurchases', data: cf.stockRepurchases },
    { label: 'Dividends Paid', data: cf.dividendsPaid },
    { label: 'Debt Repayment', data: cf.debtRepayment },
    { label: 'Stock-Based Comp', data: cf.stockBasedComp }
  ]);

  return h;
}
// ═══ HEADER RENDERERS (Company Name + Description + 4 Metric Cards) ═══
function renderStockHeader(ticker, d) {
  ...
  var cc = chg >= 0 ? C.success : C.danger;
  var bs = d.balanceSheet || {};
  var sharesArr = bs.sharesOutstanding || [];
  var shares = sharesArr.length ? sharesArr[sharesArr.length - 1].value : null;
  var mktCap = profile.marketCap || (shares && p ? shares * p : null);

  var h = '';
  // Company header card
  h += '<div class="card"><div class="card-title">' + (profile.name || ticker) + ' (' + ticker + ')' +
    (profile.sector ? ' — ' + profile.sector : '') +
    (profile.industry ? ' / ' + profile.industry : '') + '</div><div class="card-body">';
  if (profile.description) h += '<p style="font-size:12px;color:'+C.textSec+';line-height:1.6;margin-bottom:10px;">' + profile.description.substring(0, 500) + (profile.description.length > 500 ? '...' : '') + '</p>';
  h += '<div style="font-size:11px;color:'+C.textSec+';">';
  if (profile.website) h += '<strong>Website:</strong> ' + profile.website + ' &nbsp;|&nbsp; ';
  if (profile.city) h += profile.city + (profile.state ? ', ' + profile.state : '') + ' &nbsp;|&nbsp; ';
  ...
  h += '<div class="metrics-row">';
  h += mcBig(ticker, '$' + p.toFixed(2), fmtR(chgPct) + ' ($' + chg.toFixed(2) + ')', cc);
  h += mcBig('Market Cap', mktCap ? fmtB(mktCap) : '—', profile.type || '', C.textSec);
  h += mcBig('52-Week', (tech.fiftyTwoWeekLow ? '$'+tech.fiftyTwoWeekLow : '—') + ' — ' + (tech.fiftyTwoWeekHigh ? '$'+tech.fiftyTwoWeekHigh : '—'), (tech.fiftyDayAverage ? '50d: $'+tech.fiftyDayAverage : ''), C.textSec);
  h += mcBig('Volume', price.volume ? Number(price.volume).toLocaleString() : '—', 'Prev Close: $' + (price.previousClose || 0).toFixed(2), C.textSec);
  h += '</div>';
  return h;
...
window.runResearch = async function() {
  ...
  document.getElementById('researchOutput').innerHTML = '<span class="spinner"></span> Fetching <strong>' + ticker + '</strong>...';
  ...
  try {
    if (isCrypto(ticker)) {
      ...
      // Detect if ETF/Fund
      var isETF = secData.profile?.type === 'ETF' || secData.profile?.type === 'FUND';
      if (!isETF && (!secData.incomeStatement?.revenue?.length || secData.incomeStatement.revenue.length === 0)) {
        isETF = true;
      }
...
// ════════════════════════════════════════════════════
// MACRO LIVE INDICATOR TABLE — auto-fetch from FRED
// ════════════════════════════════════════════════════
(function loadMacroLiveTable() {
  fetch(WORKER_URL + "/fred").then(function(r) { return r.json(); }).then(function(d) {
    ...
    if (verdictEl) {
      ...
      vh += '<div><div style="font-size:18px;font-weight:700;color:'+pc+';">' + d.phase + '</div>';
    ...
    if (d.pillars) {
      d.pillars.forEach(function(pillar, pIdx) {
        ...
        pillar.indicators.forEach(function(ind, idx) {
          ...
          h += '<td style="padding:8px 14px;font-weight:600;">' + ind.indicator + '</td>';
    ...
    if (guideEl) {
      var gh = '<div class="card"><div class="card-title">CFA Business Cycle Phase Guide</div><div class="card-body">';
    ...
    var el = document.getElementById('macroLiveTableBody');
    if (el) el.innerHTML = '<div style="padding:24px;text-align:center;color:'+C.danger+';"><strong>Failed to load FRED data:</strong> ' + e.message + '</div>';
  });
})();
...
window.runQuantAnalysis = async function() {
  ...
  el.innerHTML = '<div style="padding:40px;text-align:center;"><span class="spinner"></span><br><br><strong>Loading SEC data for '+ticker+'...</strong></div>';
  try {
    var secData = await fetchFundamentals(ticker);
    // Render the same SEC analysis as the Research page
    var html = renderSECAnalysis(ticker, secData);
    el.innerHTML = html;
  } catch(e) {
    el.innerHTML = '<div class="card"><div class="card-title" style="background:var(--danger);">Error</div><div class="card-body"><p style="color:var(--danger);">'+e.message+'</p></div></div>';
  }
};
